...
class RegistroService
{
    ...
    public function guardar_matrimonio(?array $datos)
    {
        ...
        try {
            ...
            if (!$data) throw new \Exception('Error inesperado..');

            if ($data[0]->success !== '1') throw new \Exception($data[0]->mensaje);

            return [
                'respuesta' => $data,
            ];
        } catch (\Throwable $e) {
            log_info('RegistroService >>> guardar_matrimonio');
            log_info($e->getMessage());
            throw new \Exception('Ocurrió un error en el guardado...');
        }
    }
